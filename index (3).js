...
async function sendTokenTradePost({ state, header, subtitle, lines, type = 'LISTED' }) {
  const text = `${header}\n**${subtitle}**\n${lines.join('\n')}`;
  const opts = { parse_mode: 'Markdown', disable_notification: false, ...targetExtraForType(type) };

  const media = state.media?.token;
  if (media?.file_id) {
    if (media.kind === 'photo') return bot.telegram.sendPhoto(TARGET_CHAT_ID, media.file_id, { caption: text, ...opts });
    if (media.kind === 'animation') return bot.telegram.sendAnimation(TARGET_CHAT_ID, media.file_id, { caption: text, ...opts });
    if (media.kind === 'video') return bot.telegram.sendVideo(TARGET_CHAT_ID, media.file_id, { caption: text, ...opts });
...
async function pollKaspaComTokenTrades(state) {
  ...
  for (const r of rows) {
    const key = r._id || `${r.txid || ''}:${r.timestamp || ''}:${r.amount || ''}:${r.totalPrice || ''}`;
    if (state.tokenTrades[key]) continue;

    const sideRaw = String(r.side ?? r.type ?? r.orderType ?? '').toLowerCase();
    const isBuy = sideRaw.includes('buy');
    const isSell = sideRaw.includes('sell');

    const header =
      isBuy ? '🟢 **KASPACOM DEX BUY**' :
      isSell ? '🔴 **KASPACOM DEX SELL**' :
      '🟡 **KASPACOM DEX TRADE**';

    await sendTokenTradePost({
      state,
      header,
      subtitle: `${BONKEY_TICKER} (Kaspa.com API)`,
      lines: [
        `Amount: **${formatAmount(r.amount)} ${BONKEY_TICKER}**`,
        `Total: **${formatAmount(r.totalPrice)} KAS**`,
        r.txid ? `Tx: \`${r.txid}\`` : null,
      ].filter(Boolean),
      type: 'LISTED'
    });
...
async function tick(reason = 'auto') {
  ...
  catch (e) { await notifyAdmin(`⚠️ LEVELS ensure failed: ${e?.message ?? e}`); }

  // init DEX (once)
  try {
    if (zealousEnabled()) await initDexZealous(
      { name: 'Zealousswap', factory: ZEALOUS_FACTORY, token: ZEALOUS_TOKEN_ADDRESS, wkas: WKAS_ADDRESS },
      state.zealous
    );
  } catch (e) { await notifyAdmin(`⚠️ Zealous init failed: ${e?.message ?? e}`); }

  try {
    if (kaspacomEnabled()) await initDexStd(
      { name: 'KASPACOM', factory: KASPACOM_FACTORY, token: KASPACOM_TOKEN_ADDRESS, wkas: WKAS_ADDRESS },
      state.kaspacom
    );
  } catch (e) { await notifyAdmin(`⚠️ KaspaCom init failed: ${e?.message ?? e}`); }

  // NFT watchers
  try { await pollListings(state); } catch (e) { await notifyAdmin(`❌ Listings error: ${e?.message ?? e}`); }
  try { await pollSales(state); } catch (e) { await notifyAdmin(`❌ Sales error: ${e?.message ?? e}`); }

  // Kaspa.com token trades (API)
  try { await pollKaspaComTokenTrades(state); } catch (e) { await notifyAdmin(`❌ KaspaCom token trades error: ${e?.message ?? e}`); }

  // DEX on-chain
  try {
    if (zealousEnabled()) await pollDexZealous({
      name: 'Zealousswap',
      state,
      stateSlot: state.zealous,
      buyLink: ZEALOUS_BUY_LINK,
    });
  } catch (e) { await notifyAdmin(`❌ Zealous DEX error: ${e?.message ?? e}`); }

  try {
    if (kaspacomEnabled()) await pollDexStd({
      name: 'KASPACOM',
      state,
      stateSlot: state.kaspacom,
      buyLink: KASPACOM_BUY_LINK,
    });
  } catch (e) { await notifyAdmin(`❌ KaspaCom DEX error: ${e?.message ?? e}`); }
...
setInterval(async () => {
  try { await refreshAndCompareLevels('interval'); }
  catch (e) { await notifyAdmin(`⚠️ Levels interval failed: ${e?.message ?? e}`); }
}, Number(LEVELS_REFRESH_SEC) * 1000);

/* ===================== RUN ===================== */

bot.launch();
setInterval(() => tick('auto'), Number(POLL_INTERVAL_SEC) * 1000);
tick('boot');
